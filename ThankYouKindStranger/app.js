...
client.on('message', (message) => {
    if (!message.content.startsWith(prefix) || message.author.bot || util.isNull(message.guild))
        return;
        ...
        if (message.channel.type == 'dm')
            ...
            db.all("SELECT User, Server, Score FROM RealmGold WHERE Server = (?) ORDER BY Score DESC", server, (err, rows) => {
                ...
                for (i = 0; i < Math.min(5, rows.length); i++) {
                    ...
                    if (name == null)
                        name = message.guild.member(rows[i].User).user.username;
...
client.on('message', (message) => {
    ...
    if (command == "addimage") {
         ...
         */
        if (message.attachments.first() == null || message.author.bot)
            return;
        if (args[0] == null || typeof clowns[args[0]] == "undefined")
            return;
...
/*Quote response*/
client.on('message', (message) => {
    if (!message.content.toLowerCase().includes("keabot") || message.author.bot || util.isNull(message.guild))
        return;
    var server = message.guild.id;
    var user = message.author.id;
